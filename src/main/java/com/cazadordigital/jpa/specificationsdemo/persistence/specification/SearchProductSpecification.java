...
public class SearchProductSpecification implements Specification<Product> {
	private static final long serialVersionUID = -2047063465226577732L;
	...
	public Predicate toPredicate(Root<Product> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
		/*
		 * root: es la entidad de trabajo, en este caso Product
		 * query: es utilizado para expreciones de alto nivel como (order, group by, having)
		 * criteriaBuilder: es utilizado para las expresiones de la clausula where
		 */
		...
		if (StringUtils.hasText(name)) {
			...
			Predicate nameLikePredicate = criteriaBuilder.like(productNameToLowerCase,
					"%".concat(name.toLowerCase()).concat("%"));
			predicates.add(nameLikePredicate);
		}

		if (minPrice != null && !minPrice.equals(BigDecimal.ZERO)) {
			Predicate priceGreaterThanEqualPredicate = criteriaBuilder.greaterThanOrEqualTo(root.get("price"),
					minPrice);
		...
		if (StringUtils.hasText(category)) {
			...
			Predicate categoryNameLikePredicate = criteriaBuilder.like(categoryNameToLowerCase,
					"%".concat(category.toLowerCase()).concat("%"));
